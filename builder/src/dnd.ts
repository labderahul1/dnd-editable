...
export function onClick(event: any) {
    ...
    if (!hasSiblings( event.target)) {
        ...
        console.log(' > Nested COMPONENT, skip PROPS', event.target);
...
function changeInputProps(event: any, id: any) {
    let cardElt = <HTMLElement>document.getElementById(event.target.getAttribute("data-target"));
    console.log(event.target.value, "event", cardElt, id);
    cardElt.innerHTML = event.target.value;
}
